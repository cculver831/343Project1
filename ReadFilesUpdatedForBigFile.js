...
//-------------- READ THIS-----------------------------------------------
//this file remains the same to be transfered to the large combined file, the only thing that is diferent is the functions used to call
//the read files and to read the latest manifest number
//the updated cal methods for these functions are at the bottom of the code
//aka everything below this line (not including the function call examples) are ready to be copy and pasted into the big file
//-----------------------------------------------------------------------
...
//this is used to read all files in the directory, including files in embedded folders
//this returns all files in the folder excluding dot files ex:'.thisIsAFileName.txt' is not included
//in the final returned array
//also takes in a whole file path as a parameter to search through the whole folder (aka the folder path to search through)
returnAllFilesInDirectory("any file path")

//this is used to read the number of manifest files present in the repository and returns the num of manifest files present
//takes in the parameter of the folder holding the .Temp folder (aka the repository file path)
getLatestManifestNum("the path to the target ,holding the .Temp file")
//-------------------------------------------------------